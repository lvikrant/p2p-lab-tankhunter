...
public class ObjectController implements IObjectController {
	...
	public ObjectController(GameController gameC, int powerUpLimit,
			int tankLimit, int missileLimit, int mapID) {
		gc = gameC;
	...
	public Map<NetworkTarget, TankInfo> exportTankInfo() {
		Map<NetworkTarget, TankInfo> map = new TreeMap<NetworkTarget, TankInfo>(
				new NTComparator());

		for (Map.Entry<NetworkTarget, Tank> entry : TANK_CONTROLLER.exportMap()
				.entrySet()) {
			...
			map.put(entry.getKey(), new TankInfo(parPosX, parPosY, parAngle,
					parStatus, parTimeLeft));
		}
	...
	public void importTankInfo(Map<NetworkTarget, TankInfo> parmap) {
		...
		for (Map.Entry<NetworkTarget, TankInfo> entry : parmap.entrySet()) {
			...
			Tank tempTank = new Tank(gc, entry.getKey(), new Point(posX, posY),
					entry.getValue().getAngle());
	...
	public Map<NetworkTarget, MissileInfo> exportMissileInfo() {
		Map<NetworkTarget, MissileInfo> map = new TreeMap<NetworkTarget, MissileInfo>(
				new NTComparator());

		for (Map.Entry<NetworkTarget, Missile> entry : MISSILE_CONTROLLER
				.exportMap().entrySet()) {
			int parPosX = entry.getValue().getPosX();
			int parPosY = entry.getValue().getPosY();
			int parAngle = entry.getValue().getAngle();
			int parRange = entry.getValue().getRange();
			map.put(entry.getKey(), new MissileInfo(parPosX, parPosY, parAngle,
					parRange));
		}
	...
	public void importMissileInfo(Map<NetworkTarget, MissileInfo> parmap) {
		...
		for (Map.Entry<NetworkTarget, MissileInfo> entry : parmap.entrySet()) {
			...
			Missile tempMissile = new Missile(gc, entry.getKey(), new Point(
					posX, posY), angle, range);
	...
	public void forceAddMissile(NetworkTarget nt, Point pos, int angle,
			int range) {
	...
	public String getFieldInfo(Point point) {
		if (POWER_UP_CONTROLLER.contains(point)) {
			...
			return MAP_ELEMENTS.getFieldInfo((int) point.getX(),
					(int) point.getY());
		}
	...
	public void forceMoveTank(NetworkTarget nt, int angle, Point pos) {
		...
		switch (angle) {
		case 0:
			TANK_CONTROLLER.get(nt).moveRight();
			break;
		case 90:
			TANK_CONTROLLER.get(nt).moveUp();
			break;
		case 180:
			TANK_CONTROLLER.get(nt).moveLeft();
			break;
		case 270:
			TANK_CONTROLLER.get(nt).moveDown();
			break;
		}
	...
	public boolean moveTank(NetworkTarget nt, int angle) {
		...
		if (gc.isRegionController()) {
			 ...
			 * if(gc.getMe().equals(nt)){ //Make someone other RC of this Region
			 * 
			 * 
			 * 
			 * gc.moveToNextRegion(TANK_CONTROLLER.get(nt).getPos(), angle); }
			 * else { //Send nt to new Region
			 * 
			 * 
			 * }
			 * 
			...
			switch (angle) {
			case 0:
				ready = TANK_CONTROLLER.get(nt).moveRight();
				break;
			case 90:
				ready = TANK_CONTROLLER.get(nt).moveUp();
				break;
			case 180:
				ready = TANK_CONTROLLER.get(nt).moveLeft();
				break;
			case 270:
				ready = TANK_CONTROLLER.get(nt).moveDown();
				break;
			}
			if (ready) {
				NetworkObject no = new NetworkObject();
	...
	public void getBackupRCAck() {
		/*
		 * readyToBecomeRC = true; NetworkObject no = new NetworkObject();
		 * no.type = dataType.NewRCPing; gc.overlay.SendToOneClient(no,nt);
		 */
